...
Clustering fully unsupervised OCR'd .txt files based on semantic similarity.
Features:
- Sentence embeddings (SBERT) or TF-IDF fallback
- DBSCAN / HDBSCAN (no n_clusters needed)
- Refinement step: reassign documents to closest cluster centroid
- Compute metrics: silhouette, Davies-Bouldin, Calinski-Harabasz
- Approximate Topic Map similarity (Rafi & Shaikh 2013)
- Saves clusters.csv, metrics.json, TM similarity matrix
"""
...
class EmbeddingModel:
    def __init__(self, method="sbert", sbert_model_name="all-MiniLM-L6-v2"):
        self.method = method
        ...
        if method == "sbert":
            if not SBERT_AVAILABLE:
                raise RuntimeError("Install sentence-transformers or use method='tfidf'")
    ...
    def fit_transform(self, texts: List[str]) -> np.ndarray:
        if self.method == "sbert":
            ...
            return self.tf.fit_transform(texts).toarray()

    def transform(self, texts: List[str]) -> np.ndarray:
        if self.method == "sbert":
            return self.model.encode(texts, show_progress_bar=False, convert_to_numpy=True)
        else:
            return self.tf.transform(texts).toarray()

# ---------- TM-sim (approximation) ----------
...
def compute_tm_similarity_matrix(texts: List[str], nlp) -> np.ndarray:
    n = len(texts)
    seqs = [extract_topic_sequence(t, nlp) for t in texts]
    mat = np.zeros((n, n), dtype=float)
    for i in tqdm(range(n), desc="TM-sim pairs"):
        for j in range(i, n):
...
# ---------- Clustering ----------
def cluster_dbscan(X: np.ndarray, eps=0.5, min_samples=3, metric="cosine") -> np.ndarray:
    db = DBSCAN(eps=eps, min_samples=min_samples, metric=metric)
    labels = db.fit_predict(X)
    return labels

def cluster_hdbscan(X: np.ndarray, min_cluster_size=5, metric="euclidean") -> np.ndarray:
...
# ---------- Refinement ----------
def refine_clusters(X: np.ndarray, labels: np.ndarray) -> np.ndarray:
    """Reassign points to nearest cluster centroid for refinement"""
    ...
    for idx in range(len(labels)):
        if labels[idx] != -1:
            new_labels[idx] = unique_labels[sim[idx].argmax()]
    return new_labels
...
def compute_metrics(X: np.ndarray, labels: np.ndarray, tm_matrix: np.ndarray = None) -> Dict[str, Any]:
    metrics = {}
    ...
    if tm_matrix is not None:
        ...
        metrics['tm_diff_intra_minus_inter'] = metrics['tm_mean_intra'] - metrics['tm_mean_inter']
        metrics['tm_ratio_intra_over_inter'] = (metrics['tm_mean_intra'] / (metrics['tm_mean_inter'] + 1e-12)) if metrics['tm_mean_inter'] > 0 else float("inf")
    return metrics

# ---------- Save ----------
def save_clusters_csv(out_path: Path, docs: List[Tuple[Path,str]], labels: np.ndarray, snippet_chars=200):
    rows = []
    for (path,text), label in zip(docs, labels):
        rows.append({"filename": str(path),"cluster": int(label),"snippet": text[:snippet_chars].replace("\n"," ")})
    pd.DataFrame(rows).to_csv(out_path,index=False)
    return out_path

# ---------- Main pipeline ----------
def run_pipeline(folder: Path,
    ...
    emb_model = EmbeddingModel(method=embedding_method, sbert_model_name=sbert_model) if embedding_method=="sbert" else EmbeddingModel(method="tfidf")
    ...
    if compute_tm:
        ...
        tm_matrix = compute_tm_similarity_matrix(texts, nlp)
    ...
    return {"clusters_csv": str(csv_path),"metrics_json": str(json_path),"metrics": metrics,"labels": labels,"tm_matrix_saved": (str(output_folder / "tm_similarity_matrix.npy") if tm_matrix is not None else None)}

# ---------- CLI ----------
def main():
    parser = argparse.ArgumentParser(description="Fully unsupervised semantic clustering for OCR'd .txt files")
    ...
    res = run_pipeline(folder=args.folder,
